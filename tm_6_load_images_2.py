...
LEGENDS_FACES_DIR = "legends_faces"
COACH_FACES_DIR = "coach_faces"
LOGOS_DIR = "logos"

MAX_WORKERS = int(os.getenv("TM_IMAGES_WORKERS", "20"))
MAX_RETRIES = int(os.getenv("TM_IMAGES_MAX_RETRIES", "5"))
BIG_ATTEMPTS = int(os.getenv("TM_IMAGES_BIG_ATTEMPTS", "3"))
...
def ensure_dirs():
    ...
    os.makedirs(LEGENDS_FACES_DIR, exist_ok=True)
...
def download_as_png(url, output_path, timeout=30, max_retries=MAX_RETRIES, big_attempts=BIG_ATTEMPTS):
    if not url or is_default_image(url):
        return False

    last_exc = None
    for attempt in range(big_attempts):
        try:
            ...
            with Image.open(BytesIO(raw.getvalue())) as image:
            ...
            is_transient_http = status_code in {429, 500, 502, 503, 504}
            is_transient_net = isinstance(exc, (requests.exceptions.Timeout, requests.exceptions.ConnectionError))
            if (is_transient_http or is_transient_net) and attempt < big_attempts - 1:
...
def collect_tasks():
    ...
    legends = load_json(LEGENDS_FILE, [])
    for legend in legends:
        legend_id = legend.get("id")
        profile = legend.get("profile") or {}
        photo_url = to_high_res_player_url(profile.get("imageUrl"))
        if legend_id and photo_url and not is_default_image(photo_url):
            tasks.append({
                "kind": "player",
                "id": str(legend_id),
                "url": photo_url,
                "path": os.path.join(LEGENDS_FACES_DIR, f"{legend_id}.png"),
            })
