...
# Темно-синий фон для изображений с прозрачностью
...
def find_image_folders(root='.'):
    ...
    for name in sorted(os.listdir(root)):
        ...
        if name in SKIP_DIRS:
            continue
        if name.endswith('_compressed') or name.endswith('_compressed_transparent'):
            continue
...
def convert_image_to_jpg_dark_blue_bg(input_path, output_path):
    ...
    out.save(output_path, 'JPEG', quality=90, optimize=True)
...
def process_folder(input_folder):
    ...
    for filename in sorted(os.listdir(input_folder)):
        ...
        try:
            convert_image_to_jpg_dark_blue_bg(input_path, output_path)
            processed += 1
...
def main():
    ...
    print('\nОбработка завершена! Изображения с прозрачностью сохранены на темно-синем фоне.')
